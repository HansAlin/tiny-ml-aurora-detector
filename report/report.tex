...
\usepackage{a4wide}
\addbibresource{Tiny-ML.bib} %Imports bibliography file
...
From observations between 2024-12-01 and 2025-11-30 there were in total 8 aurora events visible from the deployment location verified by observer over 8 different dates. One verified event with no aurora, eventhogh the algorithm on the microcontroller indicated a high aurora probability, was also observed. \par
To label the data, a time window sweeping over the data from beginning of considered aurora event to end of event was applied. The time conseiderd was between 2 and 6 hours. The windows was sweeped over the data with in the consederd time at one verified dates and based on visual inspection of the light sensor with filter at 557nm. Most of the data points including a value from 557 nm sensor having a larger value than 3 was considered as aurora event. The threshold value of 3 was chosen based on visual inspection of the data from all verified aurora events. \par
Addinal labeling was done for the known non-aurora event with the same priciples mentioned previoulsy. \par
Moreover, 5 dates with high aurora points from the microcontroller algorithm, but not confirmed by obserer, where also consederd. Similar procedure were applied as above, but in order to avoid mislabeling of aurora events, a check of the magentometer located in Kiruna and Lyksele was used to varify the aurora event. Only those events with high magnetometer activity, and aurora points larger than 3 was considerd as aurora event. \par

\subsection{Data summary}
After filtering out day-time samples and applying the labeling strategy, the final dataset used for training and evaluating the deep-learning models contained approximately 1.8 M samples with unlabeled data and approximately 14,000 labeled samples, where around 13,000 samples were labeled as non-aurora and around 1,000 samples were labeled as aurora. This indicates a significant class imbalance, with non-aurora samples vastly outnumbering aurora samples. \par
Before training with data a normalization of the data was applyed using the mean and standard deviation caluclated from the training set. The normalization was done using the formula:
\[ 
...
Since the perpose of this project is to deploy deep-learning models on microcontrollers, the model architecture should be kept simple to reduce the computational cost. And for making prof of priciples a simple fully connected neural was created. \par
Since the dataset contained a lot of unlabeled data and just a small fraction of labeld data and for that matter highly un balenced, a semi-supervised learning approach was considerd, which in this context means that the models have been trained on unsuperviced data first and than part of the model fine-tuned on the labeled data. \par

This project follows to large extent the guidelines for that type of training from \cite{geronHandsonMachineLearning}. The models are created by Keras and the script describing the model creation and training is written in Python and can be found in the GitHub repository 'tiny_ml_code/models/FC_autoencoder.py' \cite{alinHansAlinTinymlauroradetector2026}.
...
\begin{itemize}
	...
	\item Encoder Hidden layer 1: Dense layer with 8 - 256 neurons, Leaky ReLU activation
	\item Encoder Hidden layer 2: Dense layer with 8 - 128 neurons, Leaky ReLU activation
	\item Encoder Latent layer: Dense layer with 2 - 64 neurons, Linear activation
	\item Decoder Hidden layer 3: Dense layer with 8 - 128 neurons, Leaky ReLU activation
	\item Decoder Hidden layer 4: Dense layer with 8 - 256 neurons, Leaky ReLU activation
...
Two types of of classifying models were coinstructe and trained in this procjetc, one using only the labeled data and one using semi-supervised learning approach. \par
\subsection{Semi-supervised learning}
The semi-supervised learning approach involved two main steps, pretraining the autoencoder model on unlabeld data and fine tuning the model on labeled data.\par
The autoencoder were able to train in 200 epochs but a callback with early stopping was used to monitor the validation loss with a patience of 10 epochs. When the model were trained the weights were saved. \par 
The classifier model "Encoder-Classifier" inherith the same archteicture as the encoder part from the previously mentioned autoencoder but with additional classifier layers. The weights from the encoder part were loaded from the trained autoencoder model. \par
Initially the weights from the encoder part were frozen as mentioned in \cite{geronHandsonMachineLearning} and the mdoel had the opurtunity to train for 200 epochs. However, the erlystopping callback prevented the model to get overfitted. When the first time in thsi classifiear training were completed, either from early stopping or when the model reache 200 epochs, the weights from the encoder part were unfrozen and the whole model were trained again for 200 epochs with early stopping. \par
